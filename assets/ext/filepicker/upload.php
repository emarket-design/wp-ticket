		...
		if (!empty($_FILES)) {
			...
			if($upload_file == 1){
				...
				if(isset($file['error'])){
					...
					if(!empty($myapp)){
						$new_file['name'] = sanitize_file_name($_FILES['file']['name']);
						$new_file['type'] = $file['type'];
						$new_file['tmp_name'] = esc_url_raw($_FILES['file']['tmp_name']);
						$new_file['size'] = intval($_FILES['file']['size']);
						...
						if(empty($sess_files[$fieldid])){
							$new_sess_files[$fieldid][]  = $new_file;
						}
